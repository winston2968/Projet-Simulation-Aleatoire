...
Cela correspond à une constante de temps \(\tau_{\text{th}} = \frac{1}{0.05} = 20\) secondes. Ce choix nous permet d'obtenir une simulation viable où la température 
...
\subsection{La gestion de la réaction : barres de contrôle}\label{subsec:comportement-barres}
...
\paragraph{La classe \texttt{ControlRod}.}
...
\begin{enumerate}
    ...
    Si le SCRAM n'est pas activé, le régulateur PI calcule l'erreur de puissance \(\varepsilon = P_{\text{consigne}} - P_{\%}\).
    Il détermine une nouvelle position cible pour les barres de régulation afin d'annuler cette erreur :
    \begin{equation}
        \text{Cible} = \text{Base} + K_p \cdot \varepsilon + K_i \cdot \int \varepsilon dt
    \end{equation}
...
\subsubsection{Algorithme de Pilotage : Le Régulateur PI !POSSIBILITE DE METTRE EN ANNEXE!}

Pour ajuster automatiquement la position des barres de régulation, nous avons implémenté un correcteur \textbf{Proportionnel-Intégral (PI)}. C'est un mécanisme de rétroaction : le système observe l'écart entre la puissance réelle et la puissance voulue pour corriger sa commande en temps réel.
...
\begin{itemize}
    \item \textbf{Le Terme Proportionnel (\(P : K_p\)) :} Il assure la réactivité du système. La correction est directement proportionnelle à l'erreur instantanée. Si l'écart de puissance est grand, la barre bouge vite.
    
    \item \textbf{Le Terme Intégral (\(I : K_i\)) :} Il assure la précision. Il correspond à la somme des erreurs passées. Si une petite erreur persiste, le terme intégral va augmenter progressivement jusqu'à forcer la barre à bouger pour éliminer totalement l'écart.
\end{itemize}
...
METTRE LE NOM DU PROJET EN ENTETE DE CETTE PAGE et faire un saut de page\\


https://fr.wikipedia.org/wiki/Fission_nucl\%C3\%A9aire\\
https://fr.wikipedia.org/wiki/R\%C3\%A9gulateur_PID\\

https://www.researchgate.net/publication/261014534_Study_of_Criticality_Safety_and_Neutronic_Performance_for_a_348-Fuel-Pin_Ghana_Research_Reactor-1_LEU_Core_Using_MCNP_Code\\
https://nuclear-twins.tacc.utexas.edu/rod_calibration\\


Son, N. , Hoa, N. , Nguyen, T. , Tuan, T. and Raul, O. (2017) Control Rod Calibration and Worth Calculation for Optimized Power Reactor 1000 (OPR-1000) Using Core Simulator OPR1000. World Journal of Nuclear Science and Technology, 7, 15-23. doi: 10.4236/wjnst.2017.71002.


James Watt. (2025, novembre 16). Wikipédia, l'encyclopédie libre. Page consultée le 15:48, novembre 16, 2025 à partir de http://fr.wikipedia.org/w/index.php?title=James_Watt&oldid=230694974.
...
\subsection{Le premier régulateur PI}

L'ancêtre du régulateur proprotionnel intégrable qui régit actuellement le monde de l'industrie vient de l'ingénieur écossais James Watt.
