...
Dans le cadre de l'UE Simulation Aléatoire de notre M1 IMA nous avons travaillé sur un projet de modélisation
de réacteur nucléaire. Nous avons donc dû, étudier, développer, simuler et étudier 
un modèle de réacteur nucléaire.
...
La troisième partie s'attarde sur les méthodes que nous avons utilisées pour implémenter le modèle ainsi que 
la structure du projet. 
Pour nous permettre de valider les attendus du projet, une étude statistique s'est avérée indispensable et consitue près de 
...
(phases rapide et épithermique) est beaucoup plus bref (cumul de 4 pas), les neutrons perdant leur énergie 
...
La dynnamique de thermalisation ci-dessus nous éclaire quand à la proportion des différents types de 
...
\begin{remarkBox}[title=Distribution Spatiale]
    L'analyse des positions $(x,y)$ des neutrons montre une concentration plus élevée au centre du réacteur, conforme 
...
L'étude des données sur les 3600 étapes de la simulation révèle un comportement dynamique marqué par d'importantes 
...
\begin{itemize}
    ...
    et une médiane légèrement inférieure à 576,0 MW. La plage de variation est très large, allant de l'arrêt complet 
    (0 MW) jusqu'à un pic de 1584 MW, ce qui indique une forte variabilité du régime de fonctionnement.
    \item Température du cœur (temperature\_k) : Ce paramètre montre une plus grande stabilité due à l'inertie 
...
\begin{remarkBox}[title=Dynamique de la Puissance et Inertie Thermique]
    ...
    thermique $C$ et du refroidissement par loi de Newton.
\end{remarkBox}
...
Une fois le signal propre obtenu, nous pouvons le confronter aux autres variables du système. Notez que pour permettre 
cette superposition visuelle, le nombre de neutrons et la puissance ont été rapportés à une échelle commune 
(normalisation), leurs ordres de grandeur bruts étant très différents.
...
La première ($p < 2.2 \times 10^{-16}$) est laa probabilité que cette répartition soit due au hasard est quasi-nulle. 
Le lien est donc statistiquement avéré. L'intensité ($\chi^2 = 906.53$), quand à elle, quantifie la force de cette relation. 
...
De plus, un test d'indépendance du $\chi^2$ réalisé entre les variations de puissance et les mouvements de barres rejette 
l'hypothèse nulle d'indépendance avec une p-value très faible ($p < 0.01$). Cela confirme statistiquement que 
notre algorithme de pilotage influence significativement le système.
...
et l'énergie produite est correct, et les commandes (les barres) fonctionnent comme prévu. Les trajets "au hasard" 
des neutrons confirment aussi que la partie probabiliste du modèle fonctionne bien.

Le seul point faible reste la stabilité : le réacteur a du mal à converger et rester à une puissance fixe. Il a tendance à 
osciller en permanence, il doit être trop sensible aux barres de contrôle, ou celles-ci en prennent pas en compte 
la dynnamique à long terme du réacteur.
...
D'autre part, ce projet nous a permis de nous plonger dans la compréhension de la physique nucléaire actuelle. 
Nous avons dû choisir quels mécanismes implémenter et comprendre les mécanismes à la base des réacteurs.
...
\begin{itemize}
    ...
    \item \textbf{Collision (\(\Sigma_t \psi\))} : Perte de neutrons due à toute interaction (absorption ou diffusion).
