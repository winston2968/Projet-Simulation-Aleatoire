...
\paragraph{Modèle Global.} Une approche générale consiste à calculer 
...
et augmenter la probabilité d'absorption \(a\) dans tout le réacteur. Etant donné que les barres de contrôle n'ont d'effet que sur
les neutrons les plus lents (\(thermiques\)), les probabilités concernant les autres types de neutrons restent inchangées.
...
grâce à la loi de refroidissement de Newton. 
...
\begin{itemize}
    ...
    \item $h$ le coefficient de transfert thermique (\(cooling\_coef\) dans notre code)
    \item $T(t)$ la temperature du réacteur à l'instant $t$
    \item $T_\text{ext}$ La température de l'environnement (ici notre eau de refroidissent à $300K$ fixé)
\end{itemize}
...
insérer le paragraph expliquant les valeurs prises :
$$C \cdot \frac{dT}{dt} = -h \cdot (T - T_{\text{eau}})$$
premier principe de la thermodynamique : diff entre ce qui rentre et ce qui sort
plus le t=0.05
et div
