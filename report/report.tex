...
\subsubsection{Types et Rôles des Barres de contrôle}
Les barres ne sont pas toutes identiques et ont chacune une vitesse de déplacement ainsi qu'une amplitude différente :\\

\begin{description}
    \item[Barres de Régulation (ou de Pilotage)] 
    ...
    \item[Barres de Compensation (ou "Shim")] 
    ...
    \item[Barres d'Arrêt d'Urgence (SCRAM)] 
    ...
    quasi instantanée en cas d'anomalie grave (ex: puissance dépassant le seuil de sécurité).\\
\end{description}


Pour des raisons pratiques, nous choisirons d'implémenter uniquement les barres de régulation ainsi que les barres SCRAM.
En effet, les barres de compensation sont utilisées sur des temps très long et pour des taches qui dépassent notre simulation.

\subsubsection{Modélisation de l'Efficacité}
L'un des défis de la modélisation de cette fonctionnalité a été que l'efficacité d'une barre n'est pas linéaire. En effet, passé le coeur du réacteur,
l'antiréactivité ne croît plus à la même vitesse. Aussi, il a fallu prendre en compte cet aspect en trouvant des alternatives.

\paragraph{Modèle Global}
Une approche générale (que nous avions d'abord implémentée) consiste à calculer un "poids" global, noté \(\rho\). 
L'efficacité de la barre en fonction de sa profondeur d'insertion \(z\) (où \(z=0\) pour "sorti" et \(z=1\) pour "inséré") 
suit une \textbf{courbe en S}, pouvant être approximée par :

$$ \rho(z) = \rho_{total} \times \frac{1}{2} (1 - \cos(\pi z)) $$

Cette antiréactivité globale \(\rho\) est ensuite utilisée pour réduire la probabilité de fission \(f\) 
...
La loi de refroidissent de Newton, énonce que le taux de perte de chaleur d'un corps est proportionnel à la différence de température entre le corps et le milieu environnant.
...
avec :
\[
    $P{\text{out}} \text{: la puissance thermique évacuée (en Watts).}$
\]

\[
    $h \text{: le coefficient de transfert thermique (cooling\_coeff).}$
\]

\[
    $T(t) \text{: la temperature du réacteur à l'instant} t$.
\]

\[
    $T_{\text{env}} \text{: La température de l'environnement (ici notre eau de refroidissent à } 300K \text{).}$
\]

l'introduire avec pq on l'a utilisé\\
...
\end{figure}
faire le lien avec le projet


Changer le dernier paragraph\\

\paragraph{Modèle Spatial}
L'approche que nous avons finalement retenue colle davantage à la réalité car elle prend en compte l'aspect spatial du réacteur. 
...
\subsubsection[Utilisation en Europe]{Qu'en est-il de notre système d'urgence européen ?}
En Europe, et particulièrement en France, le type de réacteur dominant est le \textbf{Réacteur à Eau Pressurisée} (REP).\\

Dans un REP, le contrôle de la réactivité est géré par deux systèmes complémentaires :
\begin{enumerate}
    \item \textbf{Le Bore Soluble :} De l'acide borique (un absorbeur de neutrons) est dissous dans l'eau du circuit primaire. 
    C'est un "poison" liquide qui sert de barre de compensation principale pour les ajustements lents.
    \item \textbf{Les Barres Mécaniques :} Les barres sont utilisées pour la régulation et l'arrêt d'urgence (SCRAM).
\end{enumerate}\\

Le mécanisme de SCRAM sur un REP est un exemple de \textbf{sécurité passive} : les barres d'arrêt sont maintenues en position 
...
\begin{itemize} 
    ...
    Il disparaît donc et un nombre aléatoire (suivant une loi de Poisson, \textbf{XXXX RAJOUTER LES LOISXXXXX}) de nouveaux neutrons est produit 
