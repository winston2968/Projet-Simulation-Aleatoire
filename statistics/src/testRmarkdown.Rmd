...
```
graphe de la duree de vie des neutrons A RAJOUTER CALCUL DE LA MOYENNE EXT 

```{r}
#UNIVARIE

#DUREE DE VIE DES NEUTRONS POUR CHAQUE ETAT
...
ggplot(duree_par_type, aes(x = duree_etat, fill = type)) +
  ...
  scale_y_sqrt()
```
graphe pour distinguer difference entre les 3 types de neutrons 
explications de mathis/axel le pourquoi du comment 


```{r}
#BIVARIE

#PROPORTION DE NEUTRONS DE CHAQUE TYPE POUR CHAQUE TEMPS
...
```
on peut voir qu'il y a a peut pret la meme proportion de chaque type de neutrons au fil du temps, qu'il n'y a pas de grosses différences 
...
#COUPLER AVEC LES 5 TRAJECTOIRES 
...
ggplot(traj_neutrons, aes(x = x, y = y)) +
  ...
  labs(title = "Distribution spatiale de tous les neutrons",
       ...
       fill = "NB Neutrons")
```
les neutrons sont globalement present dans tout le reacteur mais en particulier au centre où leur concentration est maximale 


```{r}
#UNIVARIE

#TRAJECTOIRE DES 5 MEILLEURS NEUTRONS
...
on a selectionne les 5 neutrons les plus vieux avec un algo pour observer leur trajectoire et essayer d'oberver pourquoi pas un pattern ou autre, mais les trajectoires ont vraiment l'air totalement aléatoire ce qui est cohérent avec notre code python. 

a regarder la trajectoire des neutrons en vrai pour une perspective d'amelioration et rendre un peu moins totalement imprevisible la trajectoire de ces derniers 
...
```
tableau de bord du reacteur brut, a voir si on mets un screen des summary de chacune de ses variables, mais au moins mettre les variances les moyennes et medianes 
...
```
graphique de transition emmenant sur le 3 pour montrer comment on passe du tableau de bord brut a une courbe linéaire assez représentative du comportement de la puissance au fil du temps 
...
graphique qui nous permet de supposer que la puissance et le nombre de neutrons est tres fortement correle, comme avec les barres de controles qui plongent quand la puissance augmente et quii se relevent quand elle diminue 
...
```
ce graphique nous montre que la position des barres de controles est tres instable malgres uen puissance stable. les seules certitudes que nous avons sont les faits que quand la puissance est tres faible(moins de 400MW), les barres sont en moyenne plus releve et que quand la puissance est au dessus du seuil de 800(seuil que nous avons mis dans le code) les barres se relevent 

si la puissance se situe entre 550 et 800, nous ne pouvons pas en deduire une position des barres de controles 

  RAJOUTER LA DROITE A 800 DE PUISSANCE MAX
...
A REVOIR CAR DIFFICILE A COMPRENDRE 

ce graphique permet de confirmer notre impression de celui de ci dessus, il est difficile de predire la postion des barres quand la puissance est moyenne, mais quand l'une des 2 variables tend vers un extreme, il y a une corrélation tres puissante 
